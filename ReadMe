...
Backend Setup
    ...
    Pull supported language images within the docker
...
Frontend Setup
    ...
    Replace the now created App.jsx, App.css, main.jsx and index.html with the files provided in the project repository of frontend. 
...
Shared Code and Language
    ...
    When a user types or changes the code, the change is emitted via Socket.IO (code_change) to the backend server.

    The server broadcasts these changes to all other connected clients.

    Similarly, when the language is changed via dropdown, the new language is emitted (language_change) and broadcasted to synchronize all users.

Running Code

    When the "Run" button is clicked:

        The current code and selected language are sent to the backend /run API.

        The backend writes the code to a file, spins up a Docker container with the appropriate image, executes the code inside the container, and streams back the output.

        The output is then broadcast to all connected clients to display in the shared output area.
